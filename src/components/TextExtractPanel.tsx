...
import { hasApiKey, getVisionModel, getTextModel } from '../lib/ai'
  ...
  const handleExtract = async () => {
    ...
    for (const item of pendingItems) {
      try {
        ...
        let prompt = ''
        let model = ''
        let content: string | { type: string; text: string } | { type: string; image_url: { url: string } }[] = ''

        if (item.text) {
          // 文字转 Markdown 模式
          model = getTextModel()
          if (item.targetFormat === 'md') {
            prompt = `请将以下文字内容转换为 Markdown 格式。要求：
1. 保持原有段落结构和标题层级
2. 使用 # ## ### 等标题标记
3. 使用 **加粗**、*斜体*等格式
4. 如果有列表，使用 - 或 1. 2. 格式
5. 如果有引用，使用 > 引用标记
6. 只输出转换后的 Markdown 内容，不要其他说明`
          } else if (item.targetFormat === 'txt') {
            prompt = `请将以下文字内容转换为纯文本格式。要求：
1. 保持段落结构和换行
2. 不要任何 Markdown 格式标记
3. 不要其他说明，只输出文字内容`
          } else {
            prompt = `请提取以下文字内容的纯文本。保持段落结构，只输出文字内容。`
          }
          content = `${prompt}\n\n输入内容：\n${item.text}`
        } else {
          // 图片 OCR 模式
...
3. 不要其他说明，只输出文字内容`
            ...
            messages: [
              {
                role: 'system',
                content: item.text
                  ? '你是一个文本格式化助手，准确地将文字转换为指定的 Markdown 格式。'
                  : '你是一个图片OCR识别助手，准确地从图片中提取文字并转换为指定格式。'
              },
              { role: 'user', content }
            ] as any,
            max_tokens: 8192,
  ...
  const handleDownload = (item: ExtractItem) => {
    ...
    a.href = url
    const fileName = item.file?.name?.replace(/[^.]+/, '') || 'text'
    a.download = `${fileName}_extracted.${item.targetFormat === 'plain' ? 'txt' : item.targetFormat}`
    a.click()
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2 pb-2 border-b">
        ...
        <h2 className="font-medium text-gray-800">文字提取与转换</h2>
        <span className="text-xs text-gray-500 ml-auto">支持图片/文字转 MD/TXT</span>
      </div>
        ...
        <div className="space-y-4">
          <div className="relative">
            ...
            <button
              ...
              className="absolute bottom-3 right-3 px-3 py-1.5 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700 disabled:bg-gray-300 flex items-center gap-1"
            >
          ...
          <div className="flex items-center gap-2 text-sm text-gray-500">
            ...
            <span>输入文字后点击"添加"，AI 将自动转换为 Markdown 格式</span>
          </div>
        </div>
      )}
                ...
                <div className="w-24 h-24 flex-shrink-0 rounded-lg overflow-hidden bg-gray-100 flex items-center justify-center">
                  {item.preview ? (
                    ...
                    <div className="text-center p-2">
                      <Type className="w-8 h-8 text-gray-400 mx-auto mb-1" />
      ...
      <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
        ...
        <ul className="text-sm text-blue-700 space-y-1">
          <li>• <strong>图片输入</strong>：拖拽图片或 Ctrl+V 粘贴截图，使用视觉模型识别</li>
          <li>• <strong>纯文字输入</strong>：直接输入或粘贴文字，AI 自动转换为 Markdown 格式</li>
